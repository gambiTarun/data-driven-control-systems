...
% the database
db(:, nu+ny+2:nu+ny+4) = K;

for i = 1:endtime
    t = round(i);

    if(t<=starttime)
        db(t, :) = [r(t+1); r(t); 0; 0; 0; 0; K(t,1); K(t,2); K(t,3); t];
    elseif(t<endtime)
        db(t, :) = [r(t+1); r(t); y0(t:-1:t-ny+1); u0(t-1:-1:t-nu+1); K(t,1); K(t,2); K(t,3); t];
    else
        db(t, :) = [0; r(t); y0(t:-1:t-ny+1); u0(t-1:-1:t-nu+1); K(t,1); K(t,2); K(t,3); t];
    end
...
for ep = 1:epochs
    ...
    fs = sqrt(var(yr-y0)/var(
    
    J_ep(ep) = 1/endtime*(sum(e.^2) + lambda*fs*del_u2);
...
ylabel('y');
xlabel('t[step]');
legend({'r', 'r1', 'y0', 'y', 'yr'}, 'Location', 'northwest');
axis([0 200 0 2.5]);
grid on;
